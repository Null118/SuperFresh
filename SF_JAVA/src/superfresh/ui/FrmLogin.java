...
public class FrmLogin extends JDialog implements ActionListener {
	...
	private JButton btnGmlog = new JButton("管理员登录");
	...
	public FrmLogin(Frame f, String s, boolean b) {
		...
		btnGmlog.addActionListener(this);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == this.btnLogin) {
			...
			try {
				...
				UsersManager.currentUser= a.login(name, pwd);
		...
		} else if (e.getSource() == this.btnGmlog) {
			String name=this.edtUserId.getText();
			String pwd=new String(this.edtPwd.getPassword());
			try {
				GmManager a = new GmManager();
				GmManager.currentGm= a.login(name, pwd);
			} catch (BaseException e1) {
				JOptionPane.showMessageDialog(null, e1.getMessage(), "错误",JOptionPane.ERROR_MESSAGE);
				return;
			}
