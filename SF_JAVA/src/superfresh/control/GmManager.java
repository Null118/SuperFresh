...
public class GmManager {
	...
	//public static BeanGm currentGm;
	...
	public void DeleteGm(BeanGm g) throws BaseException {
		...
		try {
			conn=DBUtil.getConnection();
			String sql="select * from gm where gm_name=?";
			java.sql.PreparedStatement pst=conn.prepareStatement(sql);
			pst.setString(1,g.getGm_name());
			java.sql.ResultSet rs=pst.executeQuery();
			if(!rs.next()) throw new BusinessException("请输入正确的管理员名！ ");
			rs.close();
			pst.close();
			sql = "select * from gmord,gm where gm.gm_id = gmord.gm_id and gm_name = ?";
			pst=conn.prepareStatement(sql);
			pst.setString(1,g.getGm_name());
			rs=pst.executeQuery();
			...
			rs.close();
			pst.close();
			sql = "delete from gm where gm_name = '"+g.getGm_name()+"'";
